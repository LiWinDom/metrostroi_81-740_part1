...
function ENT:Initialize()
    ...
    self:SetModel("models/metrostroi_train/81-741/body/81-741_4_physics.mdl")
	self:SetRenderMode(RENDERMODE_TRANSALPHA)
    self.BaseClass.Initialize(self)
    self:SetPos(self:GetPos() + Vector(0,0,140))

    -- Create seat entities
    self.DriverSeat = self:CreateSeat("instructor",Vector(-300,11,-35))
...
--[[
	...
	if self.SFV20.Value == 1 and self.Battery.Value == 1 then
		self:SetSkin(1)
	else
		self:SetSkin(0)
	end
	...
	if IsValid(self.FrontBogey) and IsValid(self.RearBogey) and not self.IgnoreEngine then
            ...
            self.FrontBogey.MotorForce = (17000+6500*(A < 0 and 1 or 0))--*add--35300+10000*(A < 0 and 1 or 0)
            self.FrontBogey.Reversed = self.KMR2.Value > 0
            self.FrontBogey.DisableSound = 0
            self.RearBogey.MotorForce  = (17000+6500*(A < 0 and 1 or 0))--*add--+5000--35300
